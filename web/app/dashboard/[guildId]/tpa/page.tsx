...
export default function TpaPage() {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `${NEXT_PUBLIC_BACKEND_SITE}/dashboard/thirdpartyannouncements?guildId=${guildId}`,
          { credentials: 'include' }
        );
        if (res.ok) {
  ...
  return (
    <div className="mt-6 bg-gray-700 p-4 rounded-lg shadow-md">
      ...
      <h2 className="text-2 mb-4 text-gray-400">Notifications from third party apps such as YouTube, TikTok, Twitch, sent directly to your Discord server!</h2>
