    ...
    return (<>
        ...
        <MessageCreatorEmbed
            ...
            messageAttachmentComponent={bye.card.enabled &&
                <Image
                    src={`${process.env.NEXT_PUBLIC_IMAGE_API}/welcome-card?type=welcome&username=${encodeURIComponent(user?.username as string)}&members=1090&hash=${encodeURIComponent(user?.id as string)}/${encodeURIComponent(user?.avatarHash as string)}${bye.card.background ? `&background=${encodeURIComponent(bye.card.background)}` : ""}`}
                    width={1024 / 2}
                    height={(256 + 16) / 2}
                    loading="lazy"
                    alt=""
