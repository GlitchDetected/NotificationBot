...
export function LoginHeader() {
            ...
            >
                {!user?.id ? (
                    ...
                    <>
                        ...
                        <p className="mr-1 relative bottom-[1px] truncate block">{user.displayName || user.username}</p>
                        <HiChevronDown />
                    </>
                )}
            </button>
            ...
            >
                <div className="flex items-center space-x-3 px-4 py-2">
                    ...
                    <div className="w-full">
                        ...
                        <div className="text-neutral-400 max-w-40 truncate -mt-1 text-medium sm:text-sm">@{user?.username}</div>
                    </div>
                    <button
                ...
                {buttons.map((button, i) => {
                    ...
                    if ("onChange" in button)
                        return (
                            <div key={"headerButton-" + button.name} className="flex items-center px-4 pt-2 pb-3">
                                {button.icon}
    ...
    return (
        <>
            {state === State.Failure ? <LoginButton state={state} className="ml-auto" /> : <UserButton />}

            <MotionConfig
                transition={reduceMotions ? { duration: 0 } : { type: "spring", bounce: 0.4, duration: menu ? 0.7 : 0.4 }}
            >
