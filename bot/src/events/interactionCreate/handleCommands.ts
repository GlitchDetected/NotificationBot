...
export default async (
  ...
  interaction: {
    ...
    commandName: any;
    member: { id: any; permissions: { has: (arg0: any) => any } };
    reply: (arg0: { content: string; ephemeral: boolean }) => void;
    guild: { id: any; members: { me: any } };
  }
) => {
  ...
  try {
    ...
    if (commandObject.testOnly) {
      if (!(interaction.guild.id === testServer)) {
        interaction.reply({
          content: "This command cannot be ran here. Either this command is currently in maintainence or is broken! Our developers will fix this command soon!",
          ephemeral: true
        });
        return;
      }
